...
export default function Recommand({ selections }) {
    ...
    const fetchRecommandations = async () => {
      ...
      try {
        ...
        if (res.data && res.data.course) {
          ...
          // 테마 순서에 맞게 데이터 매핑
          const mappedRecommandations = Array.from({ length: 5 }).map(() =>
            theme.map((t) => getRandomPlace(course[t]))
          );

          setRecommandations(mappedRecommandations);
        } else {
          setRecommandations([]);
        }
      } catch (error) {
  ...
  return (
    <Container>
      <Header>
        <BackButton onClick={() => navigate("/theme")}>← 다시 테마 선택</BackButton>
        <Title>추천 코스</Title>
        <Subtitle>
